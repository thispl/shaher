...
def get_invoice_data(filters):
    ...
    query = """SELECT * FROM `tabSales Invoice` WHERE {conditions} and stock_transfer != 'Stock Transfer' and status != 'Return' AND is_internal_customer = 0 """.format(conditions=conditions)
    ...
    for i in sales:
        if i.docstatus ==1:
            ...
            if i.grand_total > 0:
                ...
                if prof >= 0:
                    pro = prof
            row = [i.name,i.posting_date,i.customer,i.sales_person_user,round(i.discount_amount,2),round(i.grand_total,2),round(add,2),round(cal,2),round(pro,2),i.invoice_type,i.status,emp_name]

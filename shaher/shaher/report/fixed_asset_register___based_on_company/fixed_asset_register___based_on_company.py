...
from frappe.utils import add_months, cstr, flt, formatdate, get_first_day, getdate, nowdate, today
...
def get_data(filters):
	...
	fields = [
		...
		"total_asset_cost",
		"custom_depreciation_company_based"
	]
	...
	for asset in assets_record:
		...
		finance_book = 'Regular'
		asset_schdeuled= get_asset_depr_schedule_doc(asset.asset_id, "Active", finance_book)
		
		
		depreciation_opening = 0
		depreciation_current_year = 0
		if asset_schdeuled:
			for i in asset_schdeuled.depreciation_schedule:
				
				schedule_year = i.schedule_date.year
				if schedule_year == current_year:
					depreciation_current_year += i.depreciation_amount
					
			depreciation_opening += asset.opening_accumulated_depreciation
					
		
			depreciation_closing = depreciation_opening + depreciation_current_year
			closing_wdv = asset.total_asset_cost - depreciation_closing
			opening_wdv = asset.gross_purchase_amount - depreciation_opening
			row = {
...
def get_columns(filters):
	...
	return [
		...
		{"label": _("Qty"), "fieldtype": "Int", "fieldname": "qty", "width": 80},
		{
		...
		},
		{
			...
			"fieldname": "depreciation_per",
			"fieldtype": "Percent",
			"width": 150,
		},
		{
			"label": _("Depreciation Opening"),
			"fieldname": "depreciation_opening",
			"fieldtype": "Currency",
			"options": "Company:company:default_currency",
			"width": 180,
		},
		{
			"label": _("Depreciation Current Year"),
			"fieldname": "depreciation_cur_year",
			"fieldtype": "Currency",
			"options": "Company:company:default_currency",
			"width": 200,
		},
		{
			"label": _("Depreciation Closing"),
			"fieldname": "depreciation_closing",
			"fieldtype": "Currency",
			"options": "Company:company:default_currency",
			"width": 180,
		},
...
@frappe.whitelist()
def get_asset_depr_schedule_doc(asset_name, status, finance_book):
...
def get_asset_depr_schedule_name(asset_name, status, finance_book):

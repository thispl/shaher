...
def make_xlsx(data, sheet_name=None, wb=None, column_widths=None):
    ...
    header3 = [
        ...
        "Employee Name",
        "Designation",
        ""      # one blank column before date columns
    ]

    # Add date numbers: 01, 02, 03...
    header3.extend([d[0] for d in dates])

    # Add NEW fixed columns (NO MERGE, EXACT ORDER REQUIRED)
    header3.extend([
        "Gross Days",
        "Breakfast",
        "Lunch",
    ...
    # Second header row
    header4 = ["", "", "", "", ""]   # aligned with first 5 columns
    header4.extend([d[1] for d in dates])  # Weekdays (Sa, Su, Mo)
    header4.extend([""] * 13)  # no subheaders for fixed columns
    ...
    for emp in emp_data:
        ...
        # Attendance Row (Row 1)
        emp_row = [
            row_num,
            emp.get("name"),
            emp.get("employee_name"),
            emp.get("designation"),
            ""     # required blank column
        ]

        # Add blank date cells
        emp_row.extend([""] * len(dates))

        # Add new extra columns
        emp_row.append(len(dates))  # Gross Days
        emp_row.extend([""] * 12)   # 12 remaining new columns
        ...
        # Merge first 5 columns vertically
    ...
    merge_length = len(dates) + 17  # 1 blank + 13 new columns + 3 date extras
    ...
    # Merge fixed headers (first 5 columns)
    ...
    # Set width for date columns + new columns
    for col in range(5, merge_length + 2):  # adjust for 1-index offset
...
def get_employees(args):
    ...
    employees = frappe.db.get_all('Employee', filters=filters, fields=['name', 'employee_name','designation'])

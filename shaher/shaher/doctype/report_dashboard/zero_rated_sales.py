...
def apply_styles(ws):
	...
	for rows in ws.iter_rows(min_row=3, max_row=3, min_col=1, max_col=12):
		for cell in rows:
			...
			cell.border = border
	for rows in ws.iter_rows(min_row=4, max_row=ws.max_row-8, min_col=1, max_col=12):
		for cell in rows:
			cell.font = text_font
			cell.alignment = align_left
			cell.border = border
	for rows in ws.iter_rows(min_row=4, max_row=ws.max_row-8, min_col=5, max_col=5):
		for cell in rows:
			cell.font = text_font
	...
	for rows in ws.iter_rows(min_row=ws.max_row-6, max_row=ws.max_row-6, min_col=7, max_col=7):
		for cell in rows:
			...
			cell.alignment = align_left
			cell.border = bor
	for rows in ws.iter_rows(min_row=ws.max_row-6, max_row=ws.max_row-6, min_col=6, max_col=6):
		for cell in rows:
			cell.font = text_font
			# cell.font = Font(bold=True)
			cell.alignment = align_left
	for rows in ws.iter_rows(min_row=1, max_row=1, min_col=1, max_col=1):
		for cell in rows:
			...
			cell.font = Font(bold=True)
			# cell.alignment = align_left
	for rows in ws.iter_rows(min_row=ws.max_row, max_row=ws.max_row, min_col=1, max_col=5):
		for cell in rows:
			cell.font = text_font
...
def set_column_widths(ws):
	column_widths = [8,16] + [39] * 1 + [24, 20]  + [33, 23]  + [38] * 1 + [20, 15] +[37, 15]
...
def get_data(args):
	datas=frappe.db.sql(f"""SELECT sa.name,sa.posting_date,sa.currency,sa.grand_total,sa.customer,c.tax_id,c.customer_name,c.territory FROM `tabSales Invoice` sa LEFT JOIN `tabCustomer` c ON sa.customer = c.customer_name Where sa.docstatus=1 AND c.vat_type='Zero Vat' AND sa.posting_date BETWEEN '{args.get('from_date')}' AND '{args.get('to_date')}'""",as_dict=1)
	grant=0
	ta=0
	row =[]
	data = [
			...
			[""],
			["Serial#","Taxpayer VATIN","Taxpayer Name / Member Company Name (If applicable)","Tax Invoice/Tax Credit Note#","Tax Invoice/Tax credit note Date - DD/MM/YYYY format only","Reporting period (From DD/MM/YYYY to DD/MM/YYYY format only)"," Tax Invoice/Tax credit note Amount OMR (before VAT)","Customer Name","Customer VATIN","Customer Country","Clear description of the supply","VAT Adjustments (if any)"],
		]
	index=1
	...
	row=[""]
	data.append(row)
	row=["*Total value of zero-rated supplies of goods and services in the Sultanate, excluding exports of goods or services."]
	data.append(row)
	return data

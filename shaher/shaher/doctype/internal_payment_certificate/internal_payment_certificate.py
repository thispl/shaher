...
@frappe.whitelist()
def get_report_data(sales_order_filter=None, purchase_order_filter=None, purchase_invoice_filter=None):
	html = """
		<style>
			td {
				padding-right: 8px;
	...
	if sales_order_filter:	
		...
		for row in so.items:
			...
			sales_invoice, sales_invoice_amount, sales_invoice_date = get_sales_invoice_details(sales_order_item, sales_order)
			purchase_invoice, purchase_invoice_amount, purchase_invoice_date  = get_purchase_invoice_details(purchase_order_item, purchase_order)
			...
			if sales_invoice:
				customer_payment_entry, customer_paid_amount, customer_payment_status = get_payment_details(sales_invoice)
			if purchase_invoice:
				supplier_payment_entry, supplier_paid_amount, supplier_payment_status = get_payment_details(purchase_invoice)
			...
			html += f""""
					<tr style="color: black;">
						...
						<td class="border border-1 border-danger" style="min-width: 100px; text-align: left;">{purchase_invoice or ''}</td>
						...
						<td class="border border-1 border-danger" style="min-width: 100px; text-align: left;">{supplier_payment_entry or ''}</td>
						...
						<td class="border border-1 border-danger" style="min-width: 100px; text-align: left;">{customer_payment_entry or ''}</td>
...
def get_purchase_invoice_details(purchase_order_item, purchase_order):
	...
	name = purchase_invoice[0]['purchase_invoice'] if purchase_invoice else None
	amount = purchase_invoice[0]['amount'] if purchase_invoice else None
	date = purchase_invoice[0]['date'] if purchase_invoice else None
	return name, amount, date
...
def get_payment_details(invoice):
	...
	status = "Paid" if payment_entry else "Pending"

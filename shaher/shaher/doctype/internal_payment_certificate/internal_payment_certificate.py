...
def get_report_data(sales_order_filter=None, purchase_order_filter=None, purchase_invoice_filter=None, supplier_filter=None):
    ...
    if sales_order_filter:
        ...
        for row in so.items:
            ...
            sales_invoice_amount=float(sales_invoice_amount or 0)
            purchase_invoice_amount=float(purchase_invoice_amount or 0)
            if float(sales_invoice_amount or 0) or float(purchase_invoice_amount or 0):
                if sales_invoice_amount > 0 or purchase_invoice_amount > 0:
    ...
    if purchase_invoice_filter:
        ...
        for row in pi.items:
            ...
            sales_invoice_amount=float(sales_invoice_amount or 0)
            purchase_invoice_amount=float(purchase_invoice_amount or 0)
            sales_amt = float(sales_invoice_amount or 0)
            purchase_amt = float(purchase_invoice_amount or 0)

            base_amt = sales_amt if sales_amt > 0 else purchase_amt

            if base_amt > 0:
                profit_margin_percentage = (profit_margin / base_amt) * 100
            else:
                profit_margin_percentage = 0
...
def get_payment_details(invoice, invoice_amount):
    payment_entry = frappe.db.sql("""
                    SELECT 
                        ...
                        per.reference_doctype as reference
                    FROM
                        `tabPayment Entry` pe
                    INNER JOIN
    ...
    ref= payment_entry[0]['reference'] if payment_entry else None
    status = "Pending"
    if payment_entry:
        if ref and ref=='Sales Invoice':

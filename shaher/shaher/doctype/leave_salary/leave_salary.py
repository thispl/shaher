...
class LeaveSalary(Document):
	...
	def validate(self):
		if self.is_new():
			employee = frappe.get_doc("Employee", self.employee_number)
			if employee.custom_annual_leave_applicable_after:
				leave_gap = int(employee.custom_annual_leave_applicable_after)
				if leave_gap not in [90, 180]:
					leave_gap = 365
			else:
				leave_gap = 365
			...
			if previous_doc:
				previous_to_date = getdate(previous_doc[1])
				current_from_date = getdate(self.from_date)
				date_diff = (current_from_date - previous_to_date).days
				if date_diff < leave_gap:
					frappe.throw(_(
						f"Employee must wait {leave_gap} days between leave salary requests. "
						f"Only {date_diff} days have passed since the last document <b>{previous_doc[0]}</b>."
					))
